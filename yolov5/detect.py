...
def run(
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    new_object = {'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2, 'class': class_name.lower()}

                    dist_est = distance_estimation(bbox=new_object)
                    print(class_name)
                    print("Principle Point Ground Dist: {}".format(dist_est[0]))
                    if save_img or save_crop or view_img:  # Add bbox to image
                        c = int(cls)  # integer class
                        label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f} {int(dist_est[0])}m')
    ...
    with open("yolo_on_test_set.json", "w") as outfile:
